...
func TestQueueWrapAndGrow(t *testing.T) {
	...
	// Значения 8–11 занимают освободившиеся слоты в начале буфера — это и есть
	// заворот. На значении 12 (пятой вставке из девяти) буфер переполняется и
	// удваивается; 13–16 добавляются уже в новый буфер.
	for i := 8; i < 17; i++ {
		q.Enqueue(i)
	}

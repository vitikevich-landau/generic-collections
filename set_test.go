...
// результат независимо от порядка операндов, в том числе когда их размеры
// отличаются на порядки.
//
// Важная оговорка к имени теста: саму оптимизацию — обход меньшего множества —
// он зафиксировать не способен. Число и порядок обходов карты извне не
// наблюдаемы, поэтому реализация, всегда обходящая ресивер, прошла бы обе
// проверки ниже. Тест гарантирует только корректность и симметричность
// результата; за тем, что оптимизация на месте, следит
// BenchmarkSetIntersectSkewed.

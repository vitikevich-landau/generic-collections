...
//
// Под результат делается не более одной аллокации: место резервируется сразу
// под точную длину входа, поэтому дописывание в цикле уже не перевыделяет память.
...
// len(in)), внутренний slices.Grow выделит новый массив. Полностью без

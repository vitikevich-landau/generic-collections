...
//
// Результат выделяется ровно один раз — под точную длину входа.
...
// уже выделенный буфер (типичный приём — передать dst[:0]) и тем самым
// полностью избегает аллокаций.
//
...
func AppendMap[T, U any](dst []U, in []T, f func(T) U) []U {
	// Grow заранее расширяет ёмкость под len(in) элементов, поэтому цикл ниже
	// гарантированно не вызовет ни одного повторного выделения памяти.
...
// фильтр). Это гарантирует не более одной аллокации ценой возможного запаса
// по ёмкости — если запас нежелателен, результат можно урезать через
// slices.Clip.
...
// «на месте», полностью без аллокаций: результат пишется поверх уже прочитанных

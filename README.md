...
Все утверждения об аллокациях выше относятся только к памяти, которую выделяют
сами функции. Переданные им `f` и `keep` учитываются отдельно: например, `f`,
возвращающая на каждый элемент новый срез, добавит по аллокации на элемент
сверх одной аллокации результата.
